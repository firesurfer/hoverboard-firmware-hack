...
// Battery voltage calibration: connect power source. see <How to calibrate>. write value nr 5 to BAT_CALIB_ADC. make and flash firmware. then you can verify voltage on value 6 (devide it by 100.0f to get calibrated voltage).
...
#define FILTER              0.05f // lower value == softer filter. do not use values <0.01f, you will get float precision issues.
#define SPEED_COEFFICIENT   0.5f // higher value == stronger. 0.0f to ~2.0f?
#define STEER_COEFFICIENT   0.5f // higher value == stronger. if you do not want any steering, set it to 0.0f; 0.0f to 1.0f
...
// #define ADDITIONAL_CODE if (button1 && scale > 0.8f) { /* field weakening at high speeds */ \
